...
def extract_json_from_text(text: str):
    """
    Extract and parse JSON from a raw text / markdown string.

    Strategy (in order):
      1. Pull the content of the first ```json ... ``` or ``` ... ``` fence.
      2. Find the first top-level {...} or [...] block in the whole string.
      3. Try parsing the whole string as-is.

    Returns the parsed Python object on success, or raises ValueError.
    """
...
def extract_json_from_html(html: str):
    """
    Pull raw text out of the HTML (strips all tags) and then
    runs extract_json_from_text() on it.
    Useful when you have the raw HTML block from DeepSeek before
    any markdown conversion.
    """
    plain = re.sub(r"<[^>]+>", " ", html)          # strip tags
    ...
    return extract_json_from_text(plain)
...
class DeepSeekScraper:
    ...
    def _send_raw(self, message: str) -> tuple:
        """
        Internal: types message, waits for completion, returns
        (md_text, reasoning_blocks, elapsed, raw_html_blocks).
        raw_html_blocks is the list of raw HTML strings from the DOM
        — useful for JSON extraction without markdown corruption.
        """
        if not self.page:
        ...
        try:
            ...
            md, reasoning_blocks, raw_html_blocks = self._scrape_last_response()
            return (md, reasoning_blocks, elapsed, raw_html_blocks)
        except Exception as e:
            return (f"[Error] {e}", [], 0.0, [])

    def send_message(self, message: str) -> tuple:
        """
        Standard send — returns (md_response, reasoning_blocks, elapsed).
        Use this for display / Rich rendering as before.
        """
        md, reasoning_blocks, elapsed, _ = self._send_raw(message)
        return (md, reasoning_blocks, elapsed)

    def send_message_json(self, message: str, strict: bool = True):
        """
        Send a message and return the parsed JSON object from the response.

        Extraction order (most reliable first):
          1. Raw HTML from DOM  →  strip tags  →  find JSON
          2. Markdown text      →  find JSON

        Args:
            message: The prompt to send.
            strict:  If True (default), raises ValueError when no JSON is found.
                     If False, returns None instead of raising.

        Returns:
            Parsed Python dict / list, or None (when strict=False and nothing found).

        Example usage:
            data = scraper.send_message_json(
                'Return a JSON object with keys "name" and "score".'
            )
            print(data["name"])
        """
        md, reasoning_blocks, elapsed, raw_html_blocks = self._send_raw(message)

        # Try each raw HTML block first (avoids markdown mangling)
        for html in raw_html_blocks:
            try:
                return extract_json_from_html(html)
            except ValueError:
                pass

        # Fall back to the already-converted markdown text
        try:
            return extract_json_from_text(md)
        except ValueError as e:
            if strict:
                raise
            console.print(f"[warning]⚠  JSON extraction failed: {e}[/warning]")
            return None
    ...
    def _scrape_last_response(self) -> tuple:
        """
        Returns (response_md, reasoning_blocks: list[str], raw_html_blocks: list[str]).

        raw_html_blocks contains the raw HTML of every ds-markdown block
        so callers can extract JSON/data without going through the markdown converter.
        """
        try:
            ...
            if count <= 1:
                return (
                    self._html_to_md(texts[0]) or "[Error] Empty response",
                    [],
                    texts,  # ← raw HTML preserved
                )
            else:
                response_md      = self._html_to_md(texts[-1])
                reasoning_blocks = [self._html_to_md(h) for h in texts[:-1] if h.strip()]
                return (
                    response_md or "[Error] Empty response",
                    reasoning_blocks,
                    texts,  # ← raw HTML preserved
                )
        ...
        try:
            ...
            class _MDParser(HTMLParser):
                ...
                def __init__(self):
                    ...
                    self.list_stack=[];self.li_depth=0
